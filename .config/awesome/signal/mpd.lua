...
local function emit_info()
    awful.spawn.easy_async_with_shell(
        "sh -c 'mpc -f ARTIST@%artist%@TITLE@%title%@FILE@%file%@ && ffmpeg -y -i \"$(mpc -f $HOME/Music/%file% | head -n 1)\" /tmp/mpd_cover.jpg > /dev/null 2>&1'  ",
        function(stdout)
            local artist = stdout:match('^ARTIST@(.*)@TITLE')
            local title = stdout:match('@TITLE@(.*)@FILE')
            local file = stdout:match('@FILE@(.*)@')
            local status = stdout:match('\n%[(.*)%]')
            local album_art = "/tmp/mpd_cover.jpg"
            ...
            awesome.emit_signal("signal::mpd", artist, title, album_art, paused)
        end)
end
